...
private:
  std::unique_ptr<rev::CANSparkMax> Turn_Motor_;
  std::unique_ptr<rev::CANSparkMax> Drive_Motor_;
  std::unique_ptr<ctre::phoenix6::hardware::CANcoder> Turn_CanCoder_;

  std::unique_ptr<rev::SparkPIDController>    Drive_PID_;
  ...
  std::unique_ptr<rev::RelativeEncoder>  Drive_Encoder_;
