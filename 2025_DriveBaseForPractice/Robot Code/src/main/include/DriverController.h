...
public:
	...
	{
		using namespace frc;
		auto speeds = frc::ChassisSpeeds
		{
			-GetDBVal(Controller->GetLeftY()) * Capped_FWD_Speed,
			-GetDBVal(Controller->GetLeftX()) * Capped_STR_Speed ,
			-ApplyDeadband(Controller->GetRightX(),0.05) * Capped_RCW_Speed
		};
		frc::SmartDashboard::PutNumber("X Controller Speed", speeds.vx.value());
		frc::SmartDashboard::PutNumber("Y Controller Speed", speeds.vy.value());
	...
	frc2::Trigger Get_HandBreak_Button()
	{
		return Controller->B();
	}
